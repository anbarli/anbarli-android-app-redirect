...
final class Anbarli_Android_App_Redirect {
	...
	private function get_options(): array {
		return wp_parse_args( (array) get_option( self::OPTION_NAME, array() ), $this->defaults() );
	}
	...
	public function sanitize_options( $input ): array {
		...
		$package_name = isset( $input['package_name'] ) ? $this->sanitize_package_name( $input['package_name'] ) : '';
		$play_url     = isset( $input['play_url'] ) ? $this->sanitize_play_url( $input['play_url'] ) : '';
	...
	/**
	 * Sanitize an Android package name.
	 *
	 * @param mixed $package_name Raw package name.
	 * @return string
	 */
	private function sanitize_package_name( $package_name ): string {
		$package_name = trim( (string) $package_name );

		if ( ! preg_match( '/\A[A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z][A-Za-z0-9_]*)+\z/', $package_name ) ) {
			return '';
		}
	...
	/**
	 * Sanitize a Google Play app details URL.
	 *
	 * @param mixed $play_url Raw Google Play URL.
	 * @return string
	 */
	private function sanitize_play_url( $play_url ): string {
		$play_url = esc_url_raw( (string) $play_url, array( 'https' ) );

		if ( '' === $play_url ) {
			return '';
		}

		$host  = wp_parse_url( $play_url, PHP_URL_HOST );
		$path  = wp_parse_url( $play_url, PHP_URL_PATH );
		$query = wp_parse_url( $play_url, PHP_URL_QUERY );

		if ( 'play.google.com' !== strtolower( (string) $host ) || '/store/apps/details' !== $path || empty( $query ) ) {
			return '';
		}
	...
	public function render_banner(): void {
		...
		<script>
		(() => {
			const config = <?php echo wp_json_encode( $config, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT ); ?>;
